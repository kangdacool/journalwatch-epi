...
**중요**: 이 논문들은 이미 상위 저널(지정 핵심지 + Scimago 상위 쿼타일)에 실린 것들이다.
"그 저널에 실릴 만큼 탄탄한 논문"이라는 것만으로는 notable 기준을 충족하지 못한다 — 위 범주
중 하나에 형식적으로 해당하더라도, **그 논문이 실린 저널 자체에서 흔히 보는 전형적인 수준**이면
notable=false다. 기준은 저널마다 다르다 — 배치 전체에 걸친 고정 비율을 맞추려 하지 마라(배치
안에 최상위 저널과 상대적으로 낮은 저널이 섞여 있으면, 최상위 저널에서는 여러 편이 정당하게
notable일 수 있고 그건 정상이다). 판단 기준은 오직 "이 논문이 이 저널의 전형적 게재물보다
두드러지는가"이지, 배치 전체에서 몇 퍼센트를 뽑느냐가 아니다.
